...
import { BankTransferMerchantService } from 'src/app/services/bank_transfer_merchant.service';
...
@Component({
  selector: 'bank-transfer-merchant-management',
  templateUrl: './bank-transfer-merchant-management.component.html',
  styleUrls: ['./bank-transfer-merchant-management.component.css']
})
export class  BankTransferMerchantManagementComponent implements OnInit {
  ...
  constructor(private bankTransferMerchantService: BankTransferMerchantService, private commonHelper: CommonHelper,
  ...
  ngOnInit(): void {
    ...
    if(this.permissions.includes("bank-transfer-merchant")){
  ...
  getSlidersWithFilters(event) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.bankTransferMerchantService.getAllBankDetails(params).subscribe((res: any) => {
  ...
  onClickStatusChange(data){
    ...
    this.bankTransferMerchantService.updateStatusBankRequest(data.id,{ newStatus: newStatus }).subscribe((res: any) => {
      if (res.status == 200) {
  ...
  exportAll(){
    ...
    this.loading = true;
    this.bankTransferMerchantService.exportAllBankRequest(params).subscribe((res: any) => {
